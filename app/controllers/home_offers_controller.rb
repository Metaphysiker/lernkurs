class HomeOffersController < ApplicationController
  before_action :authenticate_user!, except: [:new, :create, :add_home_offer_to_offerer, :successfully_added_home_offer]
  before_action :is_user_allowed?, except: [:new, :create, :add_home_offer_to_offerer, :successfully_added_home_offer]
  ...
  def add_home_offer_to_offerer
    ...
    respond_to do |format|
      if @home_offer.save
        ...
        format.html { redirect_to offerer_path(@home_offer.offerer) }
      else
        format.js
      end
    end
